...
function encrypt($textToEncrypt,$method = "eof")
{
	$encryptedText;
...
{
	//$indexMethod,$configMethod, $config, $method, $text, $disencryptedText;
	$indexConfig = strpos($textToDisencrypt,'¿¡');
	$indexMethod = strrpos($textToDisencrypt, '¿¡');

	$config = substr($textToDisencrypt, ($indexConfig + 2), $indexMethod);
	$method = $dictionary[intval((substr($textToDisencrypt, ($indexMethod + 2))))];

	$text = substr($textToDisencrypt, 0, $indexConfig);
...
function encryptEzeoleaf($text)
{
	...
	$move1 = floor(rand(1, $dictionaryLength);
	do
	{
		$move2 = floor(rand(1,$dictionaryLength));
	}while($move2 == $move1);

	do
	{
		$move3 = floor(rand(1, $dictionaryLength));
	}while($move3 == $move1 || $move3 == $move2);
	
	$sign1 = floor(rand(0,1));
	$sign2 = floor(rand(0,1));
	$sign3 = floor(rand(0,1));
...
function disencryptEzeoleaf($text, $config)
{
	...
	$vConfig = explode('-', $config)
	
	$move1 = intval($vConfig[0]);
	
	$sign1 = revertSign($vConfig[1])

	for($i = 0; $i < count($text); ($i=$i+3))
	{
